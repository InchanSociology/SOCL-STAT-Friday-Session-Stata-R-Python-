...
skim(nels$expinc30)
quantile(nels$expinc30, probs = 0.40, na.rm = TRUE)
...
nels %>% 
  summarise(
    mean   = mean(expinc30, na.rm = TRUE),
    median = median(expinc30, na.rm = TRUE),
    sd     = sd(expinc30, na.rm = TRUE),
    iqr    = IQR(expinc30, na.rm = TRUE)
  )
...
mu <- mean(nels$expinc30, na.rm = TRUE)
sigma <- sd(nels$expinc30, na.rm = TRUE)
